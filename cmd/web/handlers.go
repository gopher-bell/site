...
func (app *application) userSignupPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, models.ErrDuplicateEmail) {
			...
			log.ZapLogger.Error("failed to create user", zap.Error(err))
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		}
...
func (app *application) userLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, models.ErrInvalidCredentials) {
			...
			log.ZapLogger.Error("failed to login", zap.Error(err))
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		}
	...
	if err != nil {
		log.ZapLogger.Error("failed to renew token", zap.Error(err))
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return
	}
...
func (app *application) getSessions(w http.ResponseWriter, r *http.Request) {
	a := app.sessionManager.GetString(r.Context(), "session")
	b := app.sessionManager.GetInt(r.Context(), "authenticatedUserID")
	c := strconv.Itoa(b)
	w.Write([]byte(a + "    " + c))
}
